use crate::{aredl::utils::{AREDLLevelInfo, URL_AREDL}, gd::{levels::*, utils::{Level, post_request_no_form}}};
use eyre::Error;

pub async fn get_aredl_list() -> Result<Vec<AREDLLevelInfo>, Error> {
    let url = format!("{URL_AREDL}/levels");
...
pub async fn get_aredl_level_info_by_position(position: usize) -> Result<AREDLLevelInfo, Error> {
    ...
    let url = format!("{URL_AREDL}/levels/{}", list[position - 1].id);
